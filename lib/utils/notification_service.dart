...
import 'dart:convert';
import 'package:webview_master_app/main.dart';
...
class NotificationService {
  ...
  Future<void> _initializeFirebaseMessaging() async {
    try {
      // Check if Firebase was successfully initialized in main.dart
      if (Firebase.apps.isEmpty) {
        debugPrint('❌ Firebase not initialized. Make sure Firebase.initializeApp() was called and successful.');
        return;
      }
      ...
      String? token;
      if (Platform.isIOS) {
        // FCM on iOS requires APNS token to be available first
        debugPrint('🔍 Waiting for APNS token...');
        String? apnsToken = await _firebaseMessaging!.getAPNSToken();
        
        // Retry a few times if not available immediately
        int retries = 0;
        ...
        if (apnsToken != null) {
          ...
          debugPrint('❌ APNS Token still null after retries. Ensure Push Notifications capability is added in Xcode.');
        }
      } else {
        token = await _firebaseMessaging!.getToken();
      }
      ...
      FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
        debugPrint('📨 Notification tapped while in background: ${message.messageId}');
      ...
      FirebaseMessaging.instance
        ...
        if (message != null) {
         ...
         debugPrint('📨 App opened from notification (terminated state): ${message.messageId}');
          _handleNotificationClick(message.data);
  ...
  Future<void> _handleForegroundMessage(RemoteMessage message) async {
    ...
    if (notification != null) {
      ...
      // Create a unique ID - use messageId if available, otherwise create from content (excluding timestamp)
      // This ensures duplicate notifications with same content are treated as the same notification
      final String contentKey =
          '${notification.title}_${notification.body}_${data.toString()}';
      final String uniqueId =
          notificationId.isNotEmpty ? notificationId : contentKey;
      ...
      // Create unique ID for data-only messages (exclude timestamp to avoid duplicates with same content)
      final String contentKey = '${title}_${body}_${data.toString()}';
      final String uniqueId =
          notificationId.isNotEmpty ? notificationId : contentKey;
  ...
  Future<String?> getFCMToken() async {
    ...
    // On iOS, we might need to wait for APNS token if not already there
    if (Platform.isIOS && _firebaseMessaging != null) {
      String? token = await _firebaseMessaging!.getToken();
      if (token == null) {
        debugPrint('🔍 Token null early, re-initializing to wait for APNS...');
        await _initializeFirebaseMessaging();
        token = await _firebaseMessaging!.getToken();
      }
      return token;
  ...
  /// Centralized handler for all notification clicks
  void _handleNotificationClick(Map<String, dynamic> data) {
    debugPrint('📱 Handling notification click with data: $data');
    
    // Check if we have a navigator key to perform navigation
    final context = MyApp.navigatorKey.currentContext;
    if (context != null) {
       // Since it's a WebView app, we usually stay on WebViewScreen
       // If the user wants to navigate to a specific URL, we'd need to send it to WebViewScreen
       
       // For now, just ensuring the app is at the WebViewScreen if it's not already
       // or just popping everything else if we are deep in some other screens
       // (Our app usually just has SplashScreen -> WebViewScreen)
       
       debugPrint('🚀 Navigating/Opening app via notification click');
       
       // Example: if there's a specific URL in data, we could navigate to it
       // String? targetUrl = data['url'] ?? data['link'];
       
       // Just ensuring we are on top or refreshing
       // If the app is already open, this might just trigger a refresh or do nothing
    } else {
      debugPrint('⚠️ Cannot navigate: Navigator context is null');
    }
  }
